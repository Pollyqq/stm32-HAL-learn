 ...
 *  STATIC PROTOTYPES
 **********************/
static void dht11_app_create(lv_obj_t *parent);
...
void lv_box(void)
{
    home_page = lv_tileview_create(lv_scr_act(), NULL);
#if LV_USE_THEME_MATERIAL
    if (LV_THEME_DEFAULT_INIT == lv_theme_material_init)
    {
        static lv_point_t valid_pos[] = {{0, 0}, {1, 0}, {2, 0}};
        lv_cont_set_layout(home_page, LV_LAYOUT_CENTER);
        lv_obj_set_drag_parent(home_page, true);
        lv_cont_set_layout(home_page, LV_LAYOUT_CENTER);
        lv_obj_set_size(home_page, LV_HOR_RES_MAX, LV_VER_RES_MAX);
        lv_tileview_set_edge_flash(home_page, true);
        lv_tileview_set_valid_positions(home_page, valid_pos, 3);
    }
#endif
    ...
    lv_obj_add_style(home_page, LV_CONT_PART_MAIN, &style_box);
    lv_obj_set_style_local_value_str(home_page, LV_CONT_PART_MAIN, LV_STATE_DEFAULT, "Home");

    dht11_app_create(home_page);
